...
SELECT models.name, models.brand_name, brands.headquarters FROM models INNER JOIN brands ON brands.name=models.brand_name WHERE models.name='Mustang';
...
SELECT b.name, b.founded, m.name FROM brands AS b LEFT JOIN models AS 
m ON b.name = m.brand_name WHERE b.discontinued IS NULL;
...
SELECT m.brand_name FROM models AS m GROUP BY m.brand_name HAVING COUNT(*)>5;
